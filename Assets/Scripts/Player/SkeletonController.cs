...
public class SkeletonController : MonoBehaviour, IPlayerController, IEffectable, IMovable
{
    ...
    public void SlowMovement(float percentage, float duration)
    {
        if (!isSlowed)
        {
            Debug.Log("[SkeletonController]: Slowing movement by " + percentage + "% for " + duration + "s");
    ...
    {
        Debug.Log("[SkeletonController]: Enemy was hit!");
